...
static const char *timeFormat3 = "%d/%d/%d     %d:%d";
...
esp_err_t ds3231_convertTimeToString(i2c_dev_t *dev, char* timeString, const unsigned int lenghtString)
{
    ...
    int lenght = 0;
    lenght = sprintf(timeString, timeFormat2, time.tm_mday, time.tm_mon+1, time.tm_year);
    if (lenght)
    {
...
esp_err_t ds3231_convertTimeToStringForScreen(i2c_dev_t *dev, char* timeString, const unsigned int lenghtString){
    struct tm time;
    ds3231_get_time(dev, &time);
    memset(timeString, 0, lenghtString);
    int lenght = 0;
    if(time.tm_mday < 10){
        if(time.tm_mon+1 < 10){
            if(time.tm_hour < 10){
                if(time.tm_min < 10){
                    timeFormat3 = "0%d/0%d/%d     0%d:0%d";
                }
                else{
                    timeFormat3 = "0%d/0%d/%d     0%d:%d";
                }
            }
            else{
                if(time.tm_min < 10){
                    timeFormat3 = "0%d/0%d/%d     %d:0%d";
                }
                else{
                    timeFormat3 = "0%d/0%d/%d     %d:%d";
                }
            }
        }
        else{
            if(time.tm_hour < 10){
                if(time.tm_min < 10){
                    timeFormat3 = "0%d/%d/%d     0%d:0%d";
                }
                else{
                    timeFormat3 = "0%d/%d/%d     0%d:%d";
                }
            }
            else{
                if(time.tm_min < 10){
                    timeFormat3 = "0%d/%d/%d     %d:0%d";
                }
                else{
                    timeFormat3 = "0%d/%d/%d     %d:%d";
                }
            }
        }
    }
        ...
        if(time.tm_mon+1 < 10){
            if(time.tm_hour < 10){
                if(time.tm_min < 10){
                    timeFormat3 = "%d/0%d/%d     0%d:0%d";
                }
                else{
                    timeFormat3 = "%d/0%d/%d     0%d:%d";
                }
            }
            else{
                if(time.tm_min < 10){
                    timeFormat3 = "%d/0%d/%d     %d:0%d";
                }
                else{
                    timeFormat3 = "%d/0%d/%d     %d:%d";
                }
            }
        }
        else{
            if(time.tm_hour < 10){
                if(time.tm_min < 10){
                    timeFormat3 = "%d/%d/%d     0%d:0%d";
                }
                else{
                    timeFormat3 = "%d/%d/%d     0%d:%d";
                }
            }
            else{
                if(time.tm_min < 10){
                    timeFormat3 = "%d/%d/%d     %d:0%d";
                }
                else{
                    timeFormat3 = "%d/%d/%d     %d:%d";
                }
            }
        }
    }
    lenght = sprintf(timeString, timeFormat3, time.tm_mday, time.tm_mon+1, time.tm_year, time.tm_hour, time.tm_min);
    if (lenght)
    {
        ESP_LOGI(__func__, "Convert time to string success.");
        return ESP_OK;
